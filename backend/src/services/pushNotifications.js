 ...
 */
async function generatePushCopy({ week, language = 'English', phase = 'pregnancy' }) {
  ...
  for (const week of weeks) {
    ...
    await db.query(
      `INSERT INTO push_notifications (week, language, phase, title, body, category)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (week, language, phase, category) DO UPDATE
 ...
 */
async function getPushCopy({ week, language = 'English', phase = 'pregnancy' }) {
  const { rows } = await db.query(
    `SELECT title, body, category FROM push_notifications
     WHERE week = $1 AND language = $2 AND phase = $3
